...
class AnchorHead(BaseDenseHead, BBoxTestMixin):
    ...
    def _get_targets_single(self,
        ...
        """
        ### account the number of positivate anchors
        ### and account the average of positivate anchors' number & gt_bbox every 50 imgs 
        global count_img,sum_num_gtanchor,sum_num_gtbbox
        gt_inds = assign_result.gt_inds  # 0 1 -1
        index = gt_inds > 0
        gt_inds = gt_inds[index]
        gt_anchors = anchors[index]
        count_img += 1
        sum_num_gtanchor += len(gt_inds)
        gt_len = gt_bboxes.shape[0]
        sum_num_gtbbox += gt_len
        print(len(gt_inds),gt_len)
        if count_img == 50:
            avg_num_gtanchor = sum_num_gtanchor / count_img
            avg_num_gtbbox = sum_num_gtbbox / count_img
