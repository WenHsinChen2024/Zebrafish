...
class PXRoIHead(StandardRoIHead):
    """`PointRend <https://arxiv.org/abs/1912.08193>`_."""

    def __init__(self, point_head, *args, **kwargs):
        super(PXRoIHead,self).__init__(*args, **kwargs)
    ...
    def _mask_point_forward_test(self, x, rois, label_pred, mask_pred,
        ...
        for subdivision_step in range(self.test_cfg.subdivision_steps):
            refined_mask_pred = F.interpolate(
                ...
                mode='bilinear',
                align_corners=False)
            # If `subdivision_num_points` is larger or equal to the resolution of the next step, then we can skip this step
    ...
    def forward_train(self,
        ...
        if self.with_bbox or self.with_mask:
            ...
            for i in range(num_imgs):
                ...
                iou_values = assign_result.max_overlaps ##
            ...
            for iou in all_ious:
                if 0.1 <= iou <= 0.9:
                    interval_index = int((iou - 0.1) / 0.1)
                    iou_counts[interval_index] += 1
            """        
            output_file = 'iou_counts.csv'
            with open(output_file, 'w', newline='') as csvfile:
                writer = csv.writer(csvfile)
                writer.writerow(['IoU Interval', 'Sample Count'])
                iou_thresholds = np.arange(0.1, 1.0, 0.1)


                for i, iou_threshold in enumerate(iou_thresholds):
                    interval_start = iou_threshold
                    interval_end = iou_threshold + 0.1
                    interval_count = iou_counts[i]
                    writer.writerow([iou_counts])
                    #print(f"IoU interval [{interval_start}-{interval_end}]: {interval_count} samples")
            """
            print("iou_counts",iou_counts)
            ##
            #print("iou_counts",iou_counts)
