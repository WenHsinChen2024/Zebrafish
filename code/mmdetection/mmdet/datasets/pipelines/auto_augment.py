...
class ContrastTransform:
    ...
    def _adjust_contrast_img(self, results, factor=1):
...
class Ct_Br:
    ...
    def __init__(self, Ct_level,Br_level,prob=0.5):
        assert isinstance(Ct_level, (int, float)), \
            'The level must be type int or float.'
        assert isinstance(Br_level, (int, float)), \
            'The level must be type int or float.'    
        assert 0 <= Ct_level <= _MAX_LEVEL, \
            'The level should be in range [0,_MAX_LEVEL].'
        assert 0 <= Br_level <= _MAX_LEVEL, \
            'The level should be in range [0,_MAX_LEVEL].'
        assert 0 <= prob <= 1.0, \
            'The probability should be in range [0,1].'
        self.Ct_level = Ct_level
        self.Br_level = Br_level
        self.prob = prob
        self.Ct_factor = enhance_level_to_value(Ct_level)
        self.Br_factor = enhance_level_to_value(Br_level)

    def _adjust_contrast_img(self, results, Ct_factor=1):
        """Adjust the image contrast."""
        for key in results.get('img_fields', ['img']):
            img = results[key]
            results[key] = mmcv.adjust_contrast(img, Ct_factor).astype(img.dtype)
            
    def _adjust_brightness_img(self, results, Br_factor=1):
        """Adjust the brightness of image."""
        for key in results.get('img_fields', ['img']):
            img = results[key]
            results[key] = mmcv.adjust_brightness(img, Br_factor).astype(img.dtype)
    ...
    def __call__(self, results):
        ...
        self._adjust_contrast_img(results, self.Ct_factor)
        self._adjust_brightness_img(results, self.Br_factor)
        return results

    def __repr__(self):
        repr_str = self.__class__.__name__
        repr_str += f'(level={self.Ct_level}, '
        repr_str += f'(level={self.Br_level}, '
        repr_str += f'prob={self.prob})'
        return repr_str