...
class CocoDatasetHandler:
    ...
    def polygon2shape(self, row):
        ...
        shapes = [
            [[float(points[2*i]), float(points[2*i+1])] for i in range(len(points)//2)]
    ...
    def coco2labelme(self):
        ...
        for file_idx, (filename, df) in enumerate(groups):
            ...
            for inst_idx, (_, row) in enumerate(df.iterrows()):
                # print(type(row.shapes))

                for _,p in enumerate(row.shapes):
                    pass
                #print(_)
                if _ == 0:
                    for _,polygon in enumerate(row.shapes):
...
ds = CocoDatasetHandler('./fishimage/coco_format_spine.json', './runs/our_parts/crops_spine_image') #coco.json,image
ds.coco2labelme()
ds.save_labelme(ds.labelme.keys(), './outfile/autolabel_spine')#output
