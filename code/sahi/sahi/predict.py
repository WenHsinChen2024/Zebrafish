...
from sahi.utils.cv import (
    ...
    crop_object_predictions_fish,
    cv2,
    get_video_reader,
    read_image_as_pil,
    visualize_object_predictions,
    visualize_moment_predictions,
)
...
def predict(
    ...
    visual_dir_with_moment = save_dir / "visuals_with_moment"
...
def predict_spine(
    ...
    num_=0
    coco_format=[]
    path_to_img_files = './runs/our_parts/crops_spine_image'#image
    img_file_names = [filename for filename in os.listdir(path_to_img_files)] 
    images_info = []
    i_id = 0
    for image_file_name in img_file_names:
        im = Image.open(os.path.join(path_to_img_files, image_file_name))
    ...
    # writing json file for categories
    categories_info = []
    categories_list = ["a", "b", "r"]
    for x,index in enumerate(categories_list):
    ...
    for ind, image_path in enumerate(
        ...
        if not no_sliced_prediction:
            ...
            object_prediction_list = prediction_result.object_prediction_list
            durations_in_seconds["slice"] += prediction_result.durations_in_seconds["slice"]
            # collecting annotation info
            for _ in range(len(prediction_result.to_coco_predictions(ind))):
                
                dict1=prediction_result.to_coco_predictions(ind)[_]
                dict2={'id':num_}
                dict1.update(dict2)
                annotation_info.append(dict1)
                num_+=1
        else:
            # get standard prediction
            prediction_result = get_prediction(
        ...
        if not novisual or view_video:
            ...
            output_dir_moment = str(visual_dir_with_moment / Path(relative_filepath).parent)
            result = visualize_moment_predictions(
                np.ascontiguousarray(image_as_pil),
                object_prediction_list=object_prediction_list,
                rect_th=visual_bbox_thickness,
                text_size=visual_text_size,
                text_th=visual_text_thickness,
                output_dir_moment=output_dir_moment if not source_is_video else None,
                file_name=filename_without_extension,
                export_format=visual_export_format,
            )
            if not novisual and source_is_video:  # export video
        ...
        time_end = time.time() - time_start
        durations_in_seconds["export_files"] = time_end
    coco_format.append({"images":images_info,"categories":categories_info,"annotations":annotation_info})
    with open("./fishimage/coco_format_spine.json",'w') as outputfile:
        outputfile.write(json.dumps(coco_format[0],indent=4))
    # export coco results
    if dataset_json_path:
        save_path = str(save_dir / "result.json")
        save_json(coco_json, save_path)
...
def predict_fish(
    ...
    num_=0
    coco_format=[]
    path_to_img_files = './fishimage'#image
    img_file_names = [filename for filename in os.listdir(path_to_img_files)] 
    images_info = []
    i_id = 0
    for image_file_name in img_file_names:
    ...
    # writing json file for categories
    categories_info = []
    categories_list = ["Abdomen", "Back", "Head","Spine"]
    for x,index in enumerate(categories_list):
        categories_info.append(
            {
                "supercategory": "null",
    ...
    coco_format.append({"images":images_info,"categories":categories_info,"annotations":annotation_info})
    with open("./fishimage/coco_format_fish.json",'w') as outputfile:
        outputfile.write(json.dumps(coco_format[0],indent=4))
